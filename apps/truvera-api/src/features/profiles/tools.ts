...
import type { CreateProfileRequest, UpdateProfileRequest } from "./types.js";
...
export function getHandlers(profiles: ProfilesClient): Map<string, ToolHandler> {
  ...
  handlers.set("create_profile", async (args) => formatResult(await profiles.createProfile(args as CreateProfileRequest)));
  ...
  handlers.set("update_profile", async (args) => {
    const { did, body } = args as { did?: string; body?: UpdateProfileRequest };
    if (!did) return { content: [{ type: "text", text: "Error: 'did' is required" }], isError: true };
    return formatResult(await profiles.updateProfile(did, body as UpdateProfileRequest));
  });
